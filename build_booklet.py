...
para('Reusing an ID preserves every existing file; it does not reset an attempt. Save in nano with Ctrl+O, Enter, then exit with Ctrl+X. If the browser intercepts Ctrl+X, use F2 to exit nano. Use Control on a Mac too. Keep this Codespace for review; ignored lab-work/ is not a GitHub backup.', 'small')
...
lines('Both commands use the dot. Why do they answer different questions?', 2)
